...
export const  createOrder = ({email, address,  contact, cartTotal},navigate) => async(dispatch)=>{
   try{
       ...
       dispatch({
        ...
        const response = await axios.post('http://localhost:8989/orders/', payload, config); // Adjust URL and endpoint as per your API setup
...
export const  getUserOrder = () => async(dispatch)=>{
    try{
        ...
        dispatch({
         ...
         const response = await axios.get('http://localhost:8989/orders/user-orders', config); // Adjust URL and endpoint as per your API setup
